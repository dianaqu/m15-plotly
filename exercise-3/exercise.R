...
setwd("~/Dropbox/INFO 201/m15-plotly/exercise-3")
install.packages('dplyr')
library('dplyr')

# Read in `IHME_WASHINGTON_CANCER_MORTALITY_RATES_1980_2014.csv` data using relative path
ihme.wa.maj <- read.csv('data/IHME_WASHINGTON_MORTALITY_RATES_1980_2014.csv', stringsAsFactors = FALSE)

# Remove all non-county observations from the data set 
ihme.wa.maj <- ihme.wa.maj %>% filter(location_name != 'Washington')

# Remove all observations that contain mortality rates for "both" sex
ihme.wa.maj <- ihme.wa.maj %>% filter(sex != 'Both')
...
King.County <- ihme.wa.maj %>% filter(location_name == 'King County')
...
data.after.2014 <- neoplasms.kc %>% filter(year_id > 2004)

# Using our data from 2005-2014, create a data frame that has only the 'sex', 'year_id'
# and 'mortality_rate'
filtered <- data.after.2014 %>% select(sex, year_id, mortality_rate)
...
prepped.data <- dcast(filtered, year_id ~sex)
...
part1 <- plot_ly(prepped.data, x = ~year_id, y = ~Female, type = 'bar', name = 'Female')
...
part2 <- part1 %>% add_trace(y = ~Male, name = 'Male')
...
part3 <- part2 %>% layout(title = "Neoplasms Mortality Rate in King County (2005-2014)", yaxis = list(title = 'Mortality rate'), xaxis = list(title = 'Year'), margin = list(b = 100), barmode = 'group')
...
plot_ly(prepped.data, x = ~year_id, y = ~Female, type = 'bar', name = 'Female', marker = list(color = "rgb(255,192,203)")) %>% 
  add_trace(y = ~Male, name = 'Male', marker = list(color = "rgb(135,206,250)")) %>%
  layout(title = "Neoplasms Mortality Rate in King County (2005-2014)", yaxis = list(title = 'Mortality rate'), xaxis = list(title = 'Year'), margin = list(b = 100), barmode = 'group')
